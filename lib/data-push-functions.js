...
export async function updateUserSubjectlist(id, sub_code) {
  ...
  const formattedCode = sub_code.toUpperCase();

  if (usersubjectlist.includes(formattedCode)) {
    throw new Error("Subject already exists in your list");
  }

  usersubjectlist.push(formattedCode);

  const { data: updatedData, error: updateError } = await supabase
    .from("users")
    .update({ subjects: JSON.stringify(usersubjectlist) })
    .eq("id", id);
