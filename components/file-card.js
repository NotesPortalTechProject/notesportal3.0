...
export default async function FileCard({ file, index, userid, src }) {
    ...
    // Simple device check (runs only server-side here)
    ...
    return (
        <div key={index} className="rounded-2xl bg-gradient-to-b from-[#222] to-purple-900/60 p-4 shadow-lg border border-white/10 backdrop-blur-md text-white">
            <div className="overflow-hidden rounded-xl mb-4">
                {isMobile
                    ? <div className="text-center text-white/60 py-10">click open to view more</div>
            ...
            <h2 className="text-lg font-bold tracking-wide mb-1 truncate w-full" title={file.filename}>
                {file.filename}
            </h2>
