...
export default function UploadFileModal({ children, id, subjectlist }) {
  ...
  function handleSubmit(e) {
    ...
    if (file && file.size > 50 * 1024 * 1024) {
      tempErrors.push('File size must be less than 50MB');
    }
    ...
    xhr.onload = async () => {
      ...
      try {
        ...
        if (xhr.status === 200 && data.success) {
          ...
          setSubjectcode(''); setFilename(''); setDescription(''); setFile(null);
  ...
  const modalContent = (
    <>
      <div className="fixed inset-0 z-40 backdrop-blur-sm bg-black/50" onClick={() => { setIsOpen(false); setError([]); setSuccess(''); }} />
      <div className="fixed top-1/2 left-1/2 z-50 w-[90vw] sm:w-[420px] max-w-[95vw] p-6 rounded-2xl
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-sm sm:text-base">
          ...
          <input type="text" name="subjectcode" list="subjects" value={subjectcode} onChange={(e) => setSubjectcode(e.target.value)}
          ...
          />
          <datalist id="subjects">{subjectlist.map((subj, idx) => <option key={idx} value={subj} />)}</datalist>
          ...
          {success && <div className="bg-green-500/10 border border-green-500/50 rounded-md p-2 mt-2 text-green-400 text-sm">{success}</div>}
          {errors?.length > 0 && (
            <div className="bg-red-500/10 border border-red-500/50 rounded-md p-2 mt-2 text-red-400 text-sm">
              <ul className="list-disc list-inside space-y-1">{errors.map((err, idx) => <li key={idx}>{err}</li>)}</ul>
            </div>
          )}
        </form>
      </div>
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} type="button" className="contents">{children}</button>
