      ...
      <>
        ...
        <div className="w-full pr-4 max-w-[90%] sm:max-w-[600px] md:max-w-[700px] lg:max-w-[900px] flex items-center gap-3 mt-2 mb-2 ml-4 overflow-x-auto scrollbar-hide">
          {filetypesList.map((ft, idx) => (
            <button key={idx} onClick={() => setFileType(ft)} className="flex-shrink-0">
              <div
                className={`relative overflow-hidden rounded-xl px-4 py-2 text-sm md:text-base font-medium transition-all backdrop-blur-md ${
                  filetype === ft
    ...
    <>
      ...
      <div className="w-full px-4 sm:px-6 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-4 gap-3">
          <p className="text-white/80 text-left">{filesdata.length} files available</p>
